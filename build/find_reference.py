...
#Defining the target and dataproducts
    ...
    my_dps = wp.DataProduct.select(wp.si.DataProduct.filename.regexp_match("final*"), dpowner_id=my_job.config_id) # dataproducts for the drizzled images for my_target
...
#Choosing the best reference image
    if 'reference' in my_config.parameters: # checking for set reference in parameter file
        ref_im = my_config.parameters['reference']
        my_job.logprint(f"Reference image parameter found, using {ref_im} as reference image.")

        ref_dp = wp.DataProduct.select(group = 'proc', dpowner_id = my_job.config_id, filename = ref_im) # pull correct dp for reference
...
#Firing the next event
    my_event = my_job.child_event(
        name="prep_image",
        options={
            "targname": parent_event.options["target_name"],
